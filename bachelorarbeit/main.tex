...
The following discussion is be based on the derivation by Hawking \cite{Hawking1975} and \cite{Wald1984} with insight on the derivation of the modes $f$ and $p$ gained from \cite{Donne2019}.
...
This will hold in general if the spacetime is static and flat, because if those properties are not given, we cannot readily define positive and negative frequency modes with respect to a time derivative, since the scalar product $k^\mu x_\mu $ then has mixed time and space components.
...
Thus, if we allow there to exist an asymptotically flat spacetime in the infinite past, say at $\mathscr{J}^-$, let matter accumulate over a time span such that a black hole forms, and at the end the spacetime will be asymptotically flat in the infinite future, say at $\mathscr{J}^+$, then this spacetime is definitely not static nor flat.
...
For example we could say that on $\mathscr{J}^-$, there exists a set of modes $f$, such that
\begin{align} 
  \psi =\sum_{i}^{}\left(f_ia_i+\overline{f}_ia^\dagger _i\right) \label{eq:ingoing solution}
,\end{align} 
and $\psi $ will solve the Klein-Gordon equation on $\mathscr{J}^-$.
We will choose $f$ such that they are positive frequency with respect to the affine parameter at $\mathscr{J}^-$.
Now since this set of modes is completely valid on $\mathscr{J}^-$ as it is in flat spacetime, we can use it to define $a$ and $a^\dagger $ as the annihilation and creation operator of modes on $\mathscr{J}^-$.
We can identify this with ingoing particles from $\mathscr{J}^-$.
Analoguously on $\mathscr{J}^+$ 
\begin{align} 
  \psi = \sum_{i}^{}\left(p_ib_i+\overline{p}_ib^\dagger _i\right) \label{eq:outgoing solution}
,\end{align} 
where we will identify the creation and annihilation operators $b$ and $b^\dagger $ with outgoing particles to $\mathscr{J}^+$.
...
\begin{align} 
  p_i &= \sum_{j}^{}\left(\alpha _{ij}f_j+\beta _{ij}\overline{f}_j\right)\qquad b = \sum_{j}^{}\left(\alpha _{ij}a_j+\beta _{ij}a^\dagger _j\right) \label{eq:bogolyubov transform 1}\\
  f_i &= \sum_{j}^{}\left(\overline{\alpha }_{ij}p_j-\overline{\beta }_{ij}\overline{p}_j\right)\qquad a = \sum_{j}^{}\left(\overline{\alpha }_{ij}b_j-\overline{\beta }_{ij}b^\dagger \right) \label{eq:bogolyubov transform 2}
,\end{align} 
where $\alpha $ and $\beta $ are coefficients, which fulfil
\begin{align} 
  \sum_{j}^{}\left(\alpha _{ik}\overline{\alpha }_{jk}-\beta _{ik}\overline{\beta }_{jk}\right)=\delta _{ij}\qquad \sum_{j}^{}\left(\alpha _{ik}\beta _{jk}-\beta _{ik}\alpha _{jk}\right)=0
.\end{align} 
...
As we can see from (\ref{eq:bogolyubov transform 2}), the annihilation operator for $\mathscr{J}^-$ is a linear combination of the annihilation and creation operator of $\mathscr{J}^+$.
...
We will define the vacuum on $\mathscr{J}^-$ as $\ket{0_-}$ and the vacuum at $\mathscr{J}^+$ as $\ket{0_+}$, then we have
\begin{align} 
  a\ket{0_-} &= 0\qquad a\ket{0_+}\neq 0\\
  b\ket{0_-} &\neq 0\qquad b\ket{0_+}=0
.\end{align} 
We could do even more and ask, what will be the expected number of particles in the vacuum $\ket{0_-}$, upon defining the modes on $\mathscr{J}^+$
\begin{align} 
  \bra{0_-}b^\dagger _ib_i \ket{0_-} &= \bra{0_-}\sum_{j}^{}\left(\overline{\alpha }_{ij}a^\dagger _j+\overline{\beta }_{ij}a_j\right)\left(\alpha _{ij}a_j+\beta _{ij}a^\dagger _j\right)\ket{0_-}\\
                                 &= \bra{0_-}\sum_{j}^{}\left(|\alpha _{ij}|^2a^\dagger _ja_j+\overline{\alpha }_{ij}a^\dagger _jb_{ij}a^\dagger _j+\overline{\beta }_{ij}a_j\alpha _{ij}a_j+|\beta _{ij}|^2a_ja^\dagger _j\right)\ket{0_-}\\
...
The field solves the massless Klein-Gordon equation with $\xi =0$ since we are interested in solutions on $\mathscr{J}^\pm$ 
\begin{align} 
...
We will pick $R_{\omega '}^1(v)$ since we are interested in the ingoing solutions from $\mathscr{J}^-$, i.e.\ $f$.
...
We can go through the same argumentation for the modes $p$ and we will end up with the same answer, although the time and radial part will be solved by $R_{\omega }^2(u)=P_\omega (r)\text{e}^{\text{i}\omega u}$ and be valid on $\mathscr{J}^+$.
We thus have an expression for the modes $f$ and $p$ on $\mathscr{J}^-$ and $\mathscr{J}^+$ in the Schwarzschild metric respectively
\begin{align} 
...
.\end{align} 
These modes are given in \cite{Hawking1975} equation (2.11) and (2.12), and are valid throughout the whole spacetime, since they are completely determined by their respective data on $\mathscr{J}^\pm$.

As seen from (\ref{eq:beta squared}), the number of particles on $\mathscr{J}^-$ is non-zero for an observer at $\mathscr{J}^+$ who defines the modes as $p$ and vacuum $\ket{0_+}$.
We are therefore interested in the back-propagation of $p$ to $\mathscr{J}^-$ to gain information about the waves which arrive at $\mathscr{J}^+$, i.e.\ to relate $f$ to $p$.
...
Since the black hole is not eternal, there will be a moment in the past at a time $v_0$, where geodesics going into the collapsing body will be incomplete because there is no continuation to $i^+$ or $\mathscr{J}^+$, i.e.\ the geodesic will be inside the event horizon once it has formed;
...
The modes of interest are the ones which arrive on $\mathscr{J}^-$ at a time $v\rightarrow v_0$ and went through the collapsing body on their way (the other part of the modes will just scatter back to $\mathscr{J}^-$ without going through the body).
To an observer on $\mathscr{J}^-$ the waves will be arbitrarily blueshifted as they close in on the event horizon from the future for $v\rightarrow v_0$ (as they would be redshifted if they were to come from $\mathscr{J}^-$ and reach an observer on $\mathscr{J}^+$).
...
$\mathscr{J}^-$ is a null hypersurface which is generated by null geodesics parameterized by the coordinate $v$ \todo{need show}.
We can therefore choose $v\propto \lambda |_{\mathscr{J}^-}$ on $\mathscr{J}^-$ and use this to parameterize our plane waves on $\mathscr{J}^-$.
Thereby
\begin{align} 
  p(v)=P_0\,\text{exp}\left(\text{i}\dfrac{\omega }{\kappa }\ln(-\alpha v)\right)\quad \text{on}\quad \mathscr{J}^- \quad \text{and}\quad v<0\label{eq:p on scri^-}
,\end{align} 
where $\alpha $ denotes the constant of proportionality.
This solution is only valid for $v<0$ since, on the one hand the logarithm is only defined on the positive real axis and on the other hand there exist no modes $p$ for $v>0$, since this means that $v>v_0$ for which no modes can escape to $\mathscr{J}^+$.
...
\begin{align} 
  \alpha _{\omega \omega '}=-\text{e}^{-\pi \tfrac{\omega }{\kappa }}\beta _{\omega \omega '}
.\end{align} 

\begin{enumerate}[label=--]
  \item see wald p.\ 400 and stack exchange post
  \item $f$ satisfies KG equation in schwarzschild metric (see laplace beltrami); check by computation.
    then the linear combination of the solution is valid.
  \item at asymptotic limit $f$ is a plane wave which fixes $F(r)$ to be constant (i.e.\ unity)
  \item fourier transform $\psi (\omega )$ to $\psi (v)$ gives an expression for bogolyubov coefficients (though read to p.\ 406)
\end{enumerate}
