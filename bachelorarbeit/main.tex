...
,\end{align} 
\todo{no affine parametrization formula} where $\lambda $ is the affine parameter, $\Gamma $ is the Christoffel symbol and $\nabla $ the covariant derivative.
...
\todo{formulation: distances become timelike}
One must move towards the center.
...
Therefore the radial coordinate cannot change sign, once it is determined from outside the surface; the observer has neccessarily to move in the direction of decreasing $r$ and reach the singularity at $r=0$.
We will call such a surface an \enquote{event horizon}.
...
Behind the second horizon, $l^2$ is again spacelike, as it was outside the event horizon.
Thus, if one enters the Kerr black hole from $r_+$, one has neccessarily to cross the event horizon at $r_-$, but after that is free to move however she likes.
This opens up the possibility of moving back out the black hole, as in a Kerr \enquote{white hole}, where you neccessarily have to move out of $r_+$ once you have crossed $r_-$.
...
%For future reference $\left(r_+^E\right)^2=2G^2M^2-a^2\cos ^2\theta +2GM\sqrt{GM-a^2\cos ^2\theta }$.
...
However this surface is not an event horizon, therefore $r$ is not timelike and one can move in whatever radial direction she wants.
\todo{but $\phi $ cannot be zero}

\iffalse\begin{align} 
  \Delta (r_e) &= r_e^2-2GMr_e+a^2\\
               &= 2G^2M^2-a^2\cos ^2\theta +2GM\sqrt{GM-a^2\cos ^2\theta }-2G^2M^2-2GM\sqrt{G^2M^2-a^2\cos ^2\theta }+a^2\\
               &= a^2\left(1-\cos ^2\theta \right)=a^2\sin ^2\theta \\
  \rho (r_e) &= r_e^2+a^2\cos ^2\theta =2G^2M^2-a^2\cos ^2\theta +2GM\sqrt{GM-a^2\cos ^2\theta }+a^2\cos ^2\theta \\
             &= 2G^2M^2+2GM\sqrt{GM-a^2\cos ^2\theta }\\
\left(r_e^2+a^2\right)^2 &= \left(2G^2M^2-a^2\cos ^2\theta +2GM\sqrt{GM-a^2\cos ^2\theta }+a^2\right)^2\\
                         &= \left(2G^2M^2+2GM\sqrt{GM-a^2\cos ^2\theta }+a^2\sin ^2\theta \right)^2\\
                         &= 4G^4M^4+4G^2M^2\left(GM-a^2\cos ^2\theta \right)+a^4\sin ^4\theta +4GMa^2\sin ^2\theta \sqrt{GM-a^2\cos ^2\theta }\\
                         &\qquad +4G^2M^2\left(2GM\sqrt{GM-a^2\cos ^2\theta }+a^2\sin ^2\theta \right)\\
                         &= 4G^4M^4 + 4G^3M^3\left(1+2\sqrt{GM-a^2\cos ^2\theta }\right)
\end{align}\fi
...
,\end{align} 
where $K$ is a Killing vector of the Kerr metric (\ref{eq:kerr metric}) with norm
\begin{align} 
  K^2 = K^\mu K_\mu = K^\mu g_{\mu \nu }K^\nu = g_{t t} = \dfrac{2GMr}{\rho ^2}-1 = \dfrac{2GMr}{r^2+a^2\cos ^2\theta }-1
.\end{align} 
Then
\begin{align} 
  \lim_{r \rightarrow \infty}K^2 &= -1\\
  \left.K^2\right|_{r=r_+^E} &= \dfrac{2GM\left(GM+\sqrt{G^2M^2-a^2\cos ^2\theta }\right)}{2G^2M^2-a^2\cos ^2\theta +2GM\sqrt{G^2M^2-a^2\cos ^2\theta }+a^2\cos ^2\theta }-1=0\\
  \left.K^2\right|_{r<r_+^E} &> 0
\end{align} 
...
\begin{align}
  (GM)^2=(GM_\text{irr})^2+\dfrac{(GJ)^2}{4(GM_\text{irr})^2}=(GM_\text{irr})^2+\dfrac{a^2(GM)^2}{4(GM_\text{irr})^2}
.\end{align} 
We can solve this equation for $GM_\text{irr}$ 
\begin{align} 
  (GM_\text{irr})^4-(GM_\text{irr})^2(GM)^2+\dfrac{a^2(GM)^2}{4}=0
\end{align} 
and we will only consider the \enquote{+} solution, since we want the mass within $r_+$ 
\begin{align} 
  (GM_\text{irr})^2=\dfrac{1}{2}\left( (GM)^2+\sqrt{(GM)^2\left( (GM)^2-a^2\right)}\right)=\dfrac{1}{2}GM\left(GM+\sqrt{(GM)^2-a^2}\right)=\dfrac{1}{2}GMr_+ \label{eq:irreducible mass 0}
.\end{align} 
Finally
\begin{align} 
  M_\text{irr}=\sqrt{\dfrac{Mr_+}{2G}} \label{eq:irreducible mass 1}
\end{align} 
Now we take a look at the area\todo{motivation}.
...
and
\begin{align} 
  A &= \int_{}^{}\td s'\,=\int_{}^{}\td \theta \td \phi \,\sqrt{g'}=\int_{0}^{\pi }\td \theta \,\int_{0}^{2\pi }\td \phi \,\sin \theta \left(r_+^2+a^2\right)=4\pi \left(r_+^2+a^2\right) \label{eq:kerr area 1}
.\end{align} 
