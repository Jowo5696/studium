...
If a geodesic is not affine parametrized (\ref{eq:geodesics equation}) is not zero.
...
\,.\end{align} 
which is the geodesic equation for non-affine parametrized curves \cite{Townsend1997}\footnote{Do not interchange $\kappa $ from the non-affine parametrization with $\kappa $ as the Einstein-constant.}.
The constant can also be expressed as \cite{Townsend1997} \todo{could show}
\begin{align} 
  \kappa =\sqrt{-\dfrac{1}{2}\left(\nabla ^\mu t^\nu \right)\left(\nabla _\mu t_\nu \right)}
\,.\end{align} 
\todo{If $t$ happens to be a Killing vector, i.e.\ a vector field which generates a one-parameter family of isometries, then $\kappa $ is called \enquote{surface gravity}, if it is computed on a Killing horizon. argument of proportionality on null hypersurface}

Since we will be dealing with the vacuum space around a gravitating mass, we are only interested in the solutions of the metric which fulfil the equation for $T_{\mu \nu }=\Lambda =0$\,, s.t.\
\begin{align} 
...
Although this singularity can be lifted, it is still of physical significance\footnote{As opposed to be thought by Schwarzschild himself, who regarded the singularity as an artefact of the analytical solution \cite{Schwarzschild1916}.}.
The singularity at $r=0$ is however not removable, since $R\indices{^\mu ^\nu ^\rho ^\sigma }R\indices{_\mu _\nu _\rho _\sigma }=\tfrac{48G^2M^2}{r^6}$\,, which is a coordinate independent scalar, has a singularity at that point.
...
Thus, for any light cone crossing this hypersurface, all future directed paths will point towards the centre of the black hole, since the normal vector is necessarily inside the light cone.
This is because the normal vector is also the greatest distance to traverse towards the centre (i.e.\ an upper bound), thus any other vector, whose norm is less, will also be timelike.
In general, distances become timelike and one must move towards the centre.
...
\todo{but could she not increase $\theta $ and $\phi $ so much as to cancel the negative sign of $t$?}
...
For this, we will now consider quantum field theory in curved spacetime.
...
\begin{align} 
  \dfrac{A}{4G}=S
\,.\end{align} 
\todo{black hole evaporation, area theorem not violated (carroll p.\ 417)} \todo{two other methods: unruh effect $\cdot $ wick rotation / euclidean periodicity.}
%}}}
