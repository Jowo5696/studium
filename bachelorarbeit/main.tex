...
\begin{align} 
  w\indices{^\mu }w\indices{_\mu }&>0\qq{\text{spacelike}},\\
  w\indices{^\mu }w\indices{_\mu }&=0\qq{\text{null}},\\
  w\indices{^\mu }w\indices{_\mu }&<0\qq{\text{timelike}}
.\end{align} 
...
If on $M$\,, there exists a diffeomorphism $\phi :M\rightarrow M$\,, which leaves the metric invariant, i.e.\ $\phi \indices{^*}g=g$\,, then it is called an isometry.
The vector field $K$ which generates the isometry, i.e.\ $K|_p$ tangent to a curve which goes through $\phi _0(p)$\,, is called Killing vector field and gives rise to a constant of motion along a geodesic curve \cite{Wald1984}.
Given an explicit coordinate form of a metric, one can also read off $K$ by the condition, that if $g$ does not depend on a coordinate $x\indices{^\mu }$\,, then $K\indices{^{x\indices{^\mu } }}=\partial \indices{^\mu }$ is a Killing vector.
...
A spacetime is static, if the spacetime is stationary, i.e.\ there exists a timelike Killing vector, and the metric does not have terms of the form $\td t\td x\indices{^\mu }$\,, where $t$ denotes the timelike Killing parameter.
...
Then
\begin{align} 
  l\indices{^\mu }l\indices{_\mu } &= l^\mu g_{\mu \nu }l^\nu =g^{\mu \rho }\partial_\rho Sg_{\mu \nu }g^{\nu \sigma }\partial_\sigma S=g^{\mu \rho }\partial_\rho S\partial_\mu S = g^{r r}(\partial_rS)^2 = \left(1-\dfrac{r_S}{r}\right)
\,.\end{align} 
where
\begin{align} 
...
Its norm is
\begin{align} 
  K\indices{^\mu }K\indices{_\mu }|_{r=r_S}=K\indices{^\mu }g\indices{_\mu _\nu }K\indices{^\nu }|_{r=r_S}=g\indices{_t _t}|_{r=r_S}=\left.\left(\dfrac{r_S}{r}-1\right)\right|_{r=r_S}=0
\,,\end{align} 
...
We will call a surface which exhibits these properties an \enquote{event horizon}, and the region within it a \enquote{black hole}.
...
\begin{align} 
  \diff[]{R}{T} &= \pm 1
\,.\end{align} 
This will therefore also hold at $r=r_S$; the event horizon of the black hole is also in this chart a null hypersurface.
...
The singularity at $r=0$ is however not removable.
...
\vspace{1em}
Taking the time to derive all other possible solutions for (\ref{eq:efe vacuum}) is an insurmountable task, therefore we will present the known solutions and show, where to find proofs of their uniqueness.
\subsection*{Kerr}
...
The analytical solution of the Einstein field equation in vacuum for a gravitating and charged mass is given by the Reissner-Nordström metric \cite{Carroll} \cite{Reissner1916}
\begin{align} 
...
\,.\end{align} 
where $Q$ is the electric and $P$ is the magnetic charge\footnote{For a magnetic charge the Maxwell equations take on a slightly different form. It is also believed that in some theories a magnetic charge exists. Further reading (no topic-specific literature) in \cite{Carroll} and \cite{Wald1984}.} of the body.
...
Upon replacing $2GMr\rightarrow 2GMr-G(Q^2+P^2) \inlineeqno\label{eq:kerr-newman metric}$ in (\ref{eq:kerr metric}) we arrive at the solution of (\ref{eq:efe vacuum}) for a rotating and charged mass \cite{Carroll}.

%}}}

%{{{ Black Holes
\subsection*{Black Holes} \label{sec:black holes}
\todo{here refer to solutions above and make good transition}
Black holes in general are only described by a small number of properties.
Israel \cite{Israel1967} \cite{Israel1968} was the first to deduce this property by showing that the Schwarzschild metric is the unique solution with non-singular event horizon for a static, asymptotically flat spacetime with only one gravitating mass (i.e.\ the converse of Birkhoff's theorem).
...
Analogously Robinson \cite{Robinson1975} proved the uniqueness of the Kerr metric (\ref{eq:kerr metric}) and Mazur \cite{Mazur1982} proved the uniqueness of the Kerr-Newman metric (\ref{eq:kerr-newman metric}) for their respective vacua.
...
Its norm is
\begin{align} 
  \left.K^2\right|_{r=r_+^E}=\left.g\indices{_t _t}\right|_{r=r_+^E}=0
\,.\end{align} 
...
Consider a Kerr black hole with angular momentum $J$ and a particle with four-momentum $P^\mu $\, \todo{if also has charge then just replace}.
