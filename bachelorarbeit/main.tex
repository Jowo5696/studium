...
.\end{align} 
A Lorentzian metric is a metric, whose signature is $(-,+,+,+)$, which is also the convention we will be using.
...
If $K$ also fulfils (\ref{eq:not affine geodesics}), then $\kappa $ is called surface gravity, if the equation is computed on the Killing horizon \cite{Carroll}.
...
We will set the charge to zero \todo{reason? also maybe could calc with Qneq0?}
A process to extract angular momentum of a black hole has been developed by Penrose \cite{Penrose1969} \cite{Carroll}.
We will come shortly to discussing the details of this process, first we need to understand how Kerr black holes work.
...
\begin{align} 
  r^2-2GMr+a^2 = 0\quad \Rightarrow \quad r_\pm = GM\pm \sqrt{G^2M^2-a^2} 
\,.\end{align} 
There are now three cases to consider.
$G^2M^2<a^2$\,, for this case there will be no real solution of $r_\pm$ thus no event horizon and the singularity will be a naked singularity.
This stands in violation with the cosmic censorship conjecture, therefore we will discard the solution.
$G^2M^2=a^2$\,, this is the extreme Kerr black hole, which is unstable \todo{show}.
$G^2M^2>a^2$\,, now both solutions of $r$ are real and can be of physical significance.
...
\begin{align} 
  g\indices{_t_t}=1-\dfrac{2GMr}{\rho ^2}=0\quad \Rightarrow \quad r^2-2GMr+a^2\cos ^2\theta =0\quad \Rightarrow \quad r_\pm^E=GM\pm\sqrt{G^2M^2-a^2\cos ^2\theta }
\,.\end{align} 
...
Therefore $K$ is proportional to the normal vector on this surface, however it need not be affine parametrized and will in general solve only (\ref{eq:not affine geodesics}) \todo{not true, but if K is null then K is also proportional to itself from where the non-affine geodesic equation follows}. 
The proportionality constant $\kappa $ of (\ref{eq:not affine geodesics}) is called \enquote{surface gravity} if the equation holds for a Killing vector on a Killing horizon \cite{Townsend1997} \todo{already discussed in introduction}.
...
\,.\end{align} 
where $K$ is a Killing vector of the Kerr metric (\ref{eq:kerr metric}) with norm $K^2=g_{t t}=\tfrac{2GMr}{\rho ^2}-1$ \todo{redundancy from above}
\begin{align} 
...
Since every event horizon in a stationary spacetime is a Killing horizon \cite{Carroll}, the Killing vector
\begin{align} 
...
\,.\end{align} 
Thus we have the promised relation between the quantities describing a Kerr black hole.\footnote{Do not interchange $\kappa $ (the surface gravity) with the Einstein gravitational constant, also called $\kappa $\,,  which happens to be equal to $8\pi G$\,.}
...
\begin{align} 
g\indices{^\nu ^\sigma }\left(R\indices{_\lambda _\nu }-\dfrac{1}{2}g\indices{_\lambda _\nu }R\right) &= g\indices{^\nu ^\sigma }8\pi G\,T\indices{_\lambda _\nu }\\
 \Leftrightarrow R\indices{_\lambda ^\sigma } -\dfrac{1}{2}\delta \indices*{_\lambda ^\sigma }R &= 8\pi G\,T\indices{_\lambda ^\sigma }
\,.\end{align} 
Now
\begin{align} 
...
\begin{enumerate}[label=--]
  \item in schwarzschild = 1/4GM so only for M to inf
  \item in kerr-newman 
\end{enumerate}
%}}}
