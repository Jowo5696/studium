...
If a geodesic is not affinely parameterized (\ref{eq:geodesics equation}) is not zero.
...
\,.\end{align} 
\todo{If $t$ happenes to be a Killing vector, i.e.\ a vector field which generates a one-parameter family of isometries, then $\kappa $ is called \enquote{surface gravity}, if it is computed on a Killing horizon. argument of proportionality on null hypersurface}
...
Therefore $K$ is proportional to the normal vector on this surface, however it need not be affinely parameterized and will in general solve only (\ref{eq:not affine geodesics}) \todo{not true, but if K is null then K is also proportional to itself from where the non-affine geodesic equation follows}. 
...
\,.\end{align} 
Now contract all three summands with $g\indices{^\mu ^\lambda }$ and use $t\indices{^\lambda }t\indices{_\lambda }=0$\,.
