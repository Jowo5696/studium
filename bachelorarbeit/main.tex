...
\,,\end{align} 
where $t$ is the Killing vector, which is null on the horizon.
To have $\kappa $ be the quantity that we want to compute, we impose this equation to hold only \textit{on} the horizon.
To determine, whether $\kappa $ is constant or not, we should therefore only take the derivative tangential to the horizon.
One could apply the Lie derivative to (\ref{eq:geodesic equation surface gravity}), and find out the $\kappa $ is constant on one orbit of the one-parameter family of isometries which are generated by $t$, but we want to determine whether $\kappa $ is also constant from one orbit to another (such that is constant everywhere on the horizon).
