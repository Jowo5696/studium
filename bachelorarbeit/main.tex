...
This work shall present a thorough derivation of the laws of black hole thermodynamcis.
%}}}
...
As of now, we have only dealt with classical general relativity; even in the extend to not include quantum mechanical effects into our calculations.
...
An exitation will in general be a plane wave, also called \enquote{mode}, $\psi _0\text{e}^{\text{i}k^\mu x_\mu }$, which can be understood as a particle upon approximating to first quantization.
These motion of plane waves are governed by the Klein-Gordon equation $\partial ^2\psi -m^2\psi -\xi R\psi =0$, where $m$ is the mass and $\xi $ denotes the coupling strength to the curvature scalar $R$ \cite{Carroll}.

The following discussion will be based on the derivation by Hawking \cite{Hawking1975}.
A cruicial fact is that if the modes $f$ will solve the Klein-Gordon equation, then a linear combination of these modes will also solve the Klein-Gordon equation.
...
,\end{align} 
where $\Sigma $ is a Cauchy surface, thus they form a complete set.
This will in general hold if the spacetime is static and flat, because if those properties are not given, we cannot readily define positive and negative frequency modes with respect to a time derivative, since the scalar product $k^\mu x_\mu $ will in general have mixed time and space components.
...
We will see that we are faced with exactly this problem, because in our work, we are concerned about black holes as well as their creation.
Thus, if we allow there to exist an asymptotically flat spacetime in the infinite past, say at $\mathscr{J}^-$, then let matter accumulate over a time span such that a black hole forms, and at the end the spacetime will be asymptotically flat in the infinite future, say at $\mathscr{J}^+$, then this spacetime is definitely not static nor flat.
What we can do however, is define to define a set of positive and negative frequency modes in the flat and static spacetime which is in our reach.
...
,\end{align} 
and $\psi $ will solve the Klein-Gordon equation of $\mathscr{J}^-$.
We will choose $f$ that they are positive frequency with respect to the affine parameter at $\mathscr{J}^-$.
...
\begin{align} 
  \alpha _{ij}=\left(g_i,f_j\right)\qquad \beta _{ij}=-\left(g_i,\overline{f}_j\right)
.\end{align} 
\todo{could show}
...
Remember, said curvature originates solely from our black hole and nothing else; it was therefore suspected by Hawking that a black hole radiates.

We are now tasked to calculate $|\beta _{ij}|^2$ to obtain information about the spectrum of particles.
To perform explicit calculation we will consider the Schawrzschild metric (\ref{eq:schwarzschild metric}) but in Eddington-Finkelstein coordinates
\begin{align} 
  v=t+r+2GM\ln\left(\dfrac{r}{2GM}-1\right)\qquad u=t-r-2GM\ln\left(\dfrac{r}{2GM}-1\right)
,\end{align} 
...
We will also say that, since the Schwarzschild metric is spherically symmetric, we can decompose the plane waves into spherical harmonics 
\begin{align} 
  f_{\omega '}=\dfrac{1}{\sqrt{2\pi \omega '}r}F(r)Y\left(\theta ,\phi \right)\qquad p_\omega =\dfrac{1}{\sqrt{2\pi \omega }r}P(r)Y(\theta ,\phi )
,\end{align} 
and make the summations continuous
